...
updateShootingEnemies world = world {bullets = updatedBullets, enemies = updatedEnemies}
  where
    shootingEnemies = map (updateShootIterationEnemy world) (filter (determineIfEnemyShouldShoot (iteration world)) (enemies world))
...
updateShootIterationEnemy world enemy = enemy {enemySpaceship = (enemySpaceship enemy){weapons = activeWeapon : otherWeapons}}
  where
...
determineIfEnemyShouldShoot iteration enemy
  ...
  | otherwise = False
  where lastShot = lastShotAtIteration (head (filter active (weapons (enemySpaceship enemy))))
...
updateHitPlayer bullets world player
  | foldr (\b acc -> checkIfBulletHitsPlayer player b || acc) False bullets =
    player {playerSpaceship = (playerSpaceship player) {lastHitAtIteration = iteration world, health = health (playerSpaceship player) - 10}
      }
  | otherwise = player
